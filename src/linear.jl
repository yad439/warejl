...
function setStartValues(model, schedule, problem)
	set_start_value.(model.inner[:startTime], schedule.times .- problem.carTravelTime)
	set_start_value(model.inner[:res], maximum(schedule.times .+ problem.jobLengths) - problem.carTravelTime)
